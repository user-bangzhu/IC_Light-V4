...
with block:
    with gr.Row():
        gr.Markdown(""" 完整的AI学习教程，最快最新的AI资讯，请关注匹夫网站：http://AI.cenn.com
        ### 如需帮助请联系匹夫微信：AI-pifu  """)
    with gr.Row():
        with gr.Column():
            with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Group():
                with gr.Row():
                    num_samples = gr.Slider(label="生成数量", minimum=1, maximum=12, value=1, step=1)
                    seed = gr.Number(label="种子", value=12345, precision=0)
                    randomize_seed = gr.Checkbox(label="随机种子", value=True)  # Add this line
                with gr.Row():
                    image_width = gr.Slider(label="图片宽度", minimum=256, maximum=1024, value=512, step=64)
                    image_height = gr.Slider(label="图片高度", minimum=256, maximum=1024, value=768, step=64)
                with gr.Row():
                    btn_open_outputs = gr.Button("打开输出文件夹")
                    btn_open_outputs.click(fn=open_folder)
            with gr.Accordion("高级选项", open=False):
                steps = gr.Slider(label="步数", minimum=1, maximum=100, value=20, step=1)
                cfg = gr.Slider(label="CFG ", minimum=1.0, maximum=32.0, value=7.0, step=0.01)
                highres_scale = gr.Slider(label="高分辨率比例", minimum=1.0, maximum=3.0, value=1.5, step=0.01)
                highres_denoise = gr.Slider(label="高分辨率降噪", minimum=0.1, maximum=0.9, value=0.5, step=0.01)
                a_prompt = gr.Textbox(label="添加提示", value='best quality')
                n_prompt = gr.Textbox(label="否定提示",
                                      value='lowres, bad anatomy, bad hands, cropped, worst quality')
                normal_button = gr.Button(value="细节生成 (慢4倍)")
