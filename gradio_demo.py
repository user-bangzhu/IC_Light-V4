...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            prompt = gr.Textbox(label="提示关键字")
            bg_source = gr.Radio(choices=[e.value for e in BGSource],
                                 value=BGSource.NONE.value,
                                 label="照明偏好（初始化）", type='value')
            example_quick_subjects = gr.Dataset(samples=quick_subjects, label='Subject Quick List', samples_per_page=1000, components=[prompt])
            example_quick_prompts = gr.Dataset(samples=quick_prompts, label='Lighting Quick List', samples_per_page=1000, components=[prompt])

        with gr.Column():
            result_gallery = gr.Gallery(height=768, object_fit='contain', label='Outputs')
            relight_button = gr.Button(value="生成")

            with gr.Group():
                with gr.Row():
                    num_samples = gr.Slider(label="生成图片数量", minimum=1, maximum=12, value=1, step=1)
                    seed = gr.Number(label="种子", value=12345, precision=0)
                    randomize_seed = gr.Checkbox(label="Randomize Seed", value=True)  # Add this line

                with gr.Row():
                    image_width = gr.Slider(label="图像宽度", minimum=256, maximum=1024, value=512, step=64)
                    image_height = gr.Slider(label="图像高度", minimum=256, maximum=1024, value=768, step=64)

            with gr.Accordion("高级选项", open=False):
                steps = gr.Slider(label="步数", minimum=1, maximum=100, value=25, step=1)
                cfg = gr.Slider(label="CFG规模", minimum=1.0, maximum=32.0, value=2, step=0.01)
                lowres_denoise = gr.Slider(label="低分辨率去噪 (for initial latent)", minimum=0.1, maximum=1.0, value=0.9, step=0.01)
                highres_scale = gr.Slider(label="高分辨率比例", minimum=1.0, maximum=3.0, value=1.5, step=0.01)
                highres_denoise = gr.Slider(label="高分辨率降噪", minimum=0.1, maximum=1.0, value=0.5, step=0.01)
                a_prompt = gr.Textbox(label="添加提示", value='best quality')
                n_prompt = gr.Textbox(label="否定提示", value='lowres, bad anatomy, bad hands, cropped, worst quality')
            btn_open_outputs = gr.Button("打开输出文件夹")
